...
class Parser(object):
    ...
    def forward(self, words, ext_words, tags, masks, domains, domain_id, word_lens, chars_i, wordbert, unlabel=False):
        ...
        adv = self.classfication_module(sha_out)
 
        if self.match_only:
            return matching_loss,adv
        ...
        if (self._conf.is_shared_lstm and self.use_meta):
            return arc_scores, label_scores, diff, matching_loss,adv
        elif self._conf.is_shared_lstm:
            return arc_scores, label_scores, diff,adv
        elif self.use_meta:
            return arc_scores, label_scores, matching_loss,adv
        else:
            return arc_scores, label_scores,adv
    ...
    def train_or_eval_one_batch(self, dataset, is_training, unlabel = False, eval_iter = -1):
        ...
        if unlabel == False:
            ...
            if (self.use_meta and self.match_only):
                matching_loss,adv = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            elif (self._conf.is_shared_lstm and self.use_meta):
                arc_scores, label_scores, diff, matching_loss,adv  = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            elif self._conf.is_shared_lstm:
                arc_scores, label_scores, diff,adv = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            elif self.use_meta:
                arc_scores, label_scores, matching_loss,adv = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            else:
                arc_scores, label_scores,adv = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            ...
            if (self.use_meta and self.match_only):
                matching_loss,adv = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert, unlabel)
            else:
                diff, matching_loss,adv = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert, unlabel)
            ...
            if self._conf.is_adversary:
                adv_loss = Parser.adversary_loss(adv, domains, total_word_num)
                adv_loss = self._conf.adversary_lambda_loss * adv_loss#/total_word_num
                print("adversary loss:",adv_loss)
                adv_acc = compute_domain_accuray(adv, domains)
                self._eval_metrics.adv_acc += adv_acc   
